...
# Convolution layer 2
filter_shape3 = [300, 1, 10, 10]
conv_weights3 = tf.get_variable("conv_weights3" , filter_shape3, tf.float32, tf.contrib.layers.xavier_initializer_conv2d())
conv_biases3 = tf.Variable(tf.constant(0.1, shape=[10]))
conv3 = tf.nn.conv2d(outputs_fed_lstm, conv_weights3, strides=[1,1,1,1], padding = "VALID")
...
drop = tf.nn.dropout(outputs_fed_lstm3, keep_prob = B)
# Convolution layer 3
filter_shape4 = [1,1,10,8]
...
conv4 = tf.nn.conv2d(drop, conv_weights4, strides=[1,1,1,1], padding = "VALID")
normalize4 = tf.nn.elu(conv4 + conv_biases4)
outputs_fed_lstm4 = normalize4

x = tf.squeeze(outputs_fed_lstm4, [1,2]) 
# Commented code is LSTM code.   
...
#weights = tf.Variable(tf.random_normal([300,num_classes]))
#biases  = tf.Variable(tf.random_normal([num_classes]))

#logits = tf.matmul(x, weights) + biases
    
c_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(x,Y)
loss = tf.reduce_mean(c_loss)
    

global_step = tf.Variable(0, name="global_step", trainable=False)
decayed_learning_rate = tf.train.exponential_decay(learning_rate = 0.0001,global_step = global_step,decay_steps = 50, decay_rate = 0.96, staircase = True)
optimizer= tf.train.RMSPropOptimizer(learning_rate = decayed_learning_rate , momentum = 0.9)
# Code to check whether gradients are flowing properly.
#grads = optimizer.compute_gradients(loss,[conv_weights0])
minimize_loss = optimizer.minimize(loss, global_step=global_step)   
 
correct_predict = tf.nn.in_top_k(x, Y, 1)
...
with tf.Session() as sess:
     ...
     for i in range(50):
         for j in range(10):
             ...
             probability = 0.055 # Dropout when training
         ...
         train_summary_dir = os.path.join(out_dir, "summaries2", "train")
         ...
         dev_summary_dir = os.path.join(out_dir, "summaries2", "dev")
         ...
         # Summary, Tensorboard code
         accuracy_summary = tf.scalar_summary("accuracy", accu)
         test_summary_dir = os.path.join(out_dir, "summaries2", "test")
